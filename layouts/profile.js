      ...
      <div className="lg:grid lg:grid-cols-2 lg:gap-x-8">
        ...
        <div className="lg:max-w-lg lg:self-end text-gray-500">
          <div className="flex flex-col mt-6 gap-y-3">
            {url && <div className="flex flex-row">
              <GlobeAltIcon className="h-5 w-5" />
              <ul className=" ml-2 text-sm">
                <a href={url} className="underline">{url}</a>
              </ul>
            </div>}
            {locations?.length > 0 && <div className="flex flex-row">
            ...
            {people?.length > 0 && <div className="flex flex-row">
              <UserCircleIcon className="h-5 w-5" />
              <ul className="ml-2 text-sm text-gray-500 capitalize">
                {people.map((value, index) => {
                    return <li key={index} className="inline-block mr-2">
											{value + (index === people.length - 1 ? '' : ',')}
										</li>
                  })}
              </ul>
            </div>}
            {activity?.length > 0 && <div className="flex flex-row">
            ...
            {topic?.length > 0 && <div className="flex flex-row">
        ...
        <div className="mt-10 lg:mt-0 row-span-2">
          <div className="aspect-w-1 aspect-h-1 rounded-lg overflow-hidden">
            <img src={image.cached_new ? image.cached_new : image.url} alt={title} className="w-full h-full object-center object-cover" />
          </div>
        </div>
      </div>
    </div>
