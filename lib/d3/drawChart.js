...
export const drawChart = (svgRef, orgs) => {
  ...
  const data = orgs && orgs.slice(0, 33)
    .map((org) => {
    ...
    for (const dim of dimNames) {
      ...
      // const randomish =  Math.random() * 2 / Math.pow(2, 128);
        // 0.2 +
        // parseInt(crypto.createHash("md5").update(org.title).digest("hex"), 16) /
        //   Math.pow(2, 128);
       
      // org["social_change"][dim] = parseFloat(org["social_change"][dim]) + (randomish * 4 * dotRadius) / height;
    // }
    return org;
  });
