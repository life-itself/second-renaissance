 ...
 **/
export const drawChart = (svgRef, orgs) => {
  ...
  const data = orgs.slice(0, 33).map((org) => {
    ...
    for (const dim of dimNames) {
      ...
      const randomish =
        0.2 +
        parseInt(crypto.createHash("md5").update(org.name).digest("hex"), 16) /
          Math.pow(2, 128);
  ...
  const ternaryData = data.map((d, i) => {
    const [x, y] = someTernaryPlot([d.systems, d.inner, d.cultural]);
    return {
      x,
      y,
  ...
  dots
    .on("mouseover", () => {
      return tooltip.style("display", "block").style("opacity", 1);
    })
    .on("mousemove", (event, d) => {
      const { name, regions, color } = d;
      ...
      /* Chart has a relative positioning so need to 
    ...
    .join(
      (enter) => {
        enter
          ...
          .attr(
            "transform",
            (arrow, i) => `translate(${arrow.position})rotate(${arrow.angle})`
          )
